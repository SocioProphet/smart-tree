...
fn run_snapshot(repo: PathBuf, format: String, max_depth: usize) -> Result<Value> {
    ensure_json_or_error(&format)?;
    let approved = approve_or_error(&repo)?;
...
fn ensure_json_or_error(format: &str) -> Result<()> {
    if format == "json" {
        Ok(())
    } else {
...
fn approve_or_error(repo: &Path) -> Result<ApprovedRoot> {
    approve_repo_root(repo)
}
...
fn approve_repo_root(path: &Path) -> Result<ApprovedRoot> {
    ...
    if !canonical.starts_with(&allowed_dev) {
        return Err(anyhow!("policy denied: {} is outside approved root ~/dev/**", canonical.display()));
    }
...
fn build_snapshot(approved: &ApprovedRoot, max_depth: usize) -> Result<Value> {
    ...
    Ok(json!({
        ...
        "repo_identity": {"name": approved.repo_name, "git_remote": Value::Null, "branch": branch, "commit": Value::Null},
...
fn key_files(nodes: &[FileNode], approved: &ApprovedRoot) -> Vec<Value> {
    ...
    nodes
        ...
        .filter(|node| node.path.file_name().and_then(|name| name.to_str()).map(|name| KEY_NAMES.contains(&name)).unwrap_or(false))
        .take(50)
...
fn lampstand_records_from_snapshot(snapshot: &Value, approved: &ApprovedRoot) -> Vec<Value> {
    ...
    if let Some(signals) = snapshot.get("security_signals").and_then(Value::as_array) {
        for signal in signals.iter().take(25) {
            let pattern = signal.get("pattern_name").and_then(Value::as_str).unwrap_or("security signal");
            records.push(json!({
    ...
    if let Some(candidates) = snapshot.get("memory_candidates").and_then(Value::as_array) {
        for candidate in candidates.iter().take(10) {
            records.push(json!({
                "record_type": "sourceos.lampstand.memory_candidate_record.v1",
                "title": format!("Memory candidate: {}", candidate.get("candidate_type").and_then(Value::as_str).unwrap_or("unknown")),
...
fn test_systems(nodes: &[FileNode]) -> Vec<String> {
    ...
    if nodes.iter().any(|node| {
        node.path.file_name().and_then(|name| name.to_str()).map(|name| name.contains("test") || name.contains("spec")).unwrap_or(false)
    }) {
...
fn has_file(nodes: &[FileNode], name: &str) -> bool {
    nodes.iter().any(|node| !node.is_dir && node.path.file_name().and_then(|file_name| file_name.to_str()).map(|file_name| file_name == name).unwrap_or(false))
}
...
fn policy_decision(capabilities: Vec<&str>) -> Value {
    json!({"decision": "allow", "ruleset": POLICY_PROFILE, "capabilities": capabilities, "redactions": []})
}
...
fn relative_path_ref(path: &Path, root: &Path) -> String {
    path.strip_prefix(root).map(|relative| relative.display().to_string()).unwrap_or_else(|_| path_ref(path))
}
...
fn redacted_match(matched_text: &str) -> Value {
    if matched_text.is_empty() { Value::Null } else { json!(format!("[redacted:{} chars]", matched_text.chars().count())) }
}
...
fn is_adapter_error(value: &Value) -> bool {
    value.get("schema_version") == Some(&json!("sourceos.adapter_error.v1"))
}
