...
fn is_sensitive_path(path: &Path, home: &Path) -> bool {
    ...
    exact_denied.iter().any(|denied| path == denied)
        || prefix_denied.iter().any(|denied| path.starts_with(denied))
}
...
fn lampstand_records_from_snapshot(snapshot: &Value, approved: &ApprovedRoot) -> Vec<Value> {
    ...
    records.push(json!({
        ...
        "source_root_id": Value::Null,
        "path_ref": approved.path_ref.clone(),
    ...
    if let Some(signals) = snapshot.get("security_signals").and_then(|value| value.as_array()) {
        for signal in signals.iter().take(25) {
            ...
            records.push(json!({
                ...
                "source_root_id": Value::Null,
                "path_ref": signal.get("path_ref").cloned().unwrap_or_else(|| json!(approved.path_ref.clone())),
                "content_hash": Value::Null,
